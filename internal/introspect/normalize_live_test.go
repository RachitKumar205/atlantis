...
func TestNormalizePredicate_RealPostgres(t *testing.T) {
  ...
  is_default boolean
	...
	cases := []struct {
		...
		{"streq", "status = 'active'", cmpPred("=", "status", strLit("active"))},
		{"intgt", "tier > 3", cmpPred(">", "tier", intLit(3))},
		{"and", "status = 'active' AND deleted_at IS NULL",
			exprPred("status = 'active' AND deleted_at IS NULL", "status", "deleted_at")},
		{"or-commuted", "deleted_at IS NULL OR status = 'active'",
			exprPred("status = 'active' OR deleted_at IS NULL", "status", "deleted_at")},
		{"barebool", "is_default", exprPred("is_default", "is_default")},
		{"in", "tier IN (1, 2, 3)", exprPred("tier IN (3, 1, 2)", "tier")},
		{"func", "lower(status) = 'x'", exprPred("lower(status) = 'x'", "status")},
		{"coalesce", "coalesce(status, 'n') = 'x'", exprPred("coalesce(status, 'n') = 'x'", "status")},
		{"cast", "tier::bigint > 3", exprPred("tier::bigint > 3", "tier")},
		{"case", "(CASE WHEN is_default THEN tier ELSE 0 END) > 0",
			exprPred("(CASE WHEN is_default THEN tier ELSE 0 END) > 0", "is_default", "tier")},
		// genuinely-everything: constructs the hand-rolled grammar never had.
		{"arithmetic", "tier * 2 > 4", exprPred("tier * 2 > 4", "tier")},
		{"like", "status LIKE 'a%'", exprPred("status LIKE 'a%'", "status")},
		{"between", "tier BETWEEN 1 AND 10", exprPred("tier BETWEEN 1 AND 10", "tier")},
		{"concat", "status || 'x' = 'ax'", exprPred("status || 'x' = 'ax'", "status")},
		{"any-array", "tier = ANY (ARRAY[1, 2, 3])", exprPred("tier = ANY (ARRAY[1, 2, 3])", "tier")},
		{"is-distinct", "tier IS DISTINCT FROM 5", exprPred("tier IS DISTINCT FROM 5", "tier")},
	}
