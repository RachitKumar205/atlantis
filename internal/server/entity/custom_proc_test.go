...
func TestBuildSnapshot_PopulatesProcMeta(t *testing.T) {
	ir := rawProcIR("DeleteStaging",
		...
		rawStep("DELETE FROM vendor.staging WHERE sync_job_id = $sync_job_id", "vendor.VendorStagingProduct"),
	)
	...
	if len(pm.touched) != 1 || pm.touched[0] != "vendor.VendorStagingProduct" {
