...
func newTestSession(t *testing.T) *session {
	...
	open := &OpenSession{
		Queue:       "default",
		JobNames:    []string{"vendor.BulkImport"},
		MaxInFlight: 4,
		PodID:       "test-pod",
	}
	return newSession(open, "vendor")
}
...
func TestSession_RecordDispatchDecrementsAvailable(t *testing.T) {
	s := newTestSession(t)
	d := &Dispatch{JobID: 1, JobName: "vendor.BulkImport"}
...
func TestSession_MaxInFlightClamped(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			s := newSession(&OpenSession{Queue: "q", JobNames: []string{"x"}, MaxInFlight: tc.in}, "c")
...
func TestSession_HandlesJob(t *testing.T) {
	s := newTestSession(t)
	if !s.handlesJob("vendor.BulkImport") {
