...
func TestCheckWorkerAuthz_MatchingCaller(t *testing.T) {
	err := CheckWorkerAuthz("vendor", []string{"vendor.BulkImport"}, newTestIR())
	if err != nil {
...
func TestCheckWorkerAuthz_WildcardVisibleTo(t *testing.T) {
	...
	err := CheckWorkerAuthz("worker", []string{"consumer.SweepExpired"}, newTestIR())
	if err != nil {
...
func TestCheckWorkerAuthz_MismatchedCaller(t *testing.T) {
	err := CheckWorkerAuthz("worker", []string{"vendor.BulkImport"}, newTestIR())
	if err == nil {
...
func TestCheckWorkerAuthz_MixedJobsRejectFirstMismatch(t *testing.T) {
	...
	err := CheckWorkerAuthz("vendor", []string{"vendor.BulkImport", "consumer.SweepExpired"}, newTestIR())
	if err != nil {
		// Both are visible-to-vendor (wildcard), so OK.
		t.Errorf("vendor + wildcard should pass: %v", err)
	}
	err = CheckWorkerAuthz("vendor", []string{"vendor.BulkImport", "shop.OtherUnknown"}, newTestIR())
	if err == nil {
...
func TestCheckSingleAuthz_Wraps(t *testing.T) {
	...
	err := CheckSingleAuthz("worker", "vendor.BulkImport", newTestIR())
	if err == nil {
