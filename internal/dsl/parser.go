...
func (p *Parser) parseIndex() *IndexDecl {
	...
	case TokPartial:
		...
		fields := p.parseIndexFields()
		p.expect(TokWhere)
		// The lexer captured the predicate verbatim as one TokString (it is a
		// SQL expression, parsed + validated by Postgres at IR lowering).
		body := p.expect(TokString)
		return &IndexDecl{Pos: kw.Pos, Kind: IndexPartial, Fields: fields,
			WhereRaw: body.Value, WherePos: body.Pos}
