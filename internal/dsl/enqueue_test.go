...
procedure TriggerImport for vendor.Vendor {
  ...
  steps {
    enqueue vendor.ShopifyImport(vendor_id: $vendor_id)
  }
}
`)
	if len(f.Decls) != 3 {
	...
	if eq.Target.Namespace != "vendor" || eq.Target.Name != "ShopifyImport" {
...
`)
	...
	if step.Enqueue.TargetJobID != "vendor.ShopifyImport" {
		t.Errorf("target = %s", step.Enqueue.TargetJobID)
	}
	if step.Enqueue.Queue != "shopify" {
