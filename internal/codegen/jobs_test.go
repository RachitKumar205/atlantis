...
`)
	...
	for _, want := range []string{
		"package vendorpkg",
		"type ShopifyImportArgs struct",
		"VendorId",
		"ImportStrategy",
		"`json:\"vendor_id\"`",
		"`json:\"import_strategy\"`",
		"type ShopifyImportHandler interface",
		"Handle(ctx context.Context, args ShopifyImportArgs) error",
		`const ShopifyImportJobName = "vendor.ShopifyImport"`,
		"func RegisterShopifyImport(reg *jobs.Registry, h ShopifyImportHandler)",
		"jobs.HandlerFunc",
